...
- **시각적 규칙**: 입력된 데이터가 없는 빈 상태는 `text-gray-400` 색상과 `italic` (기울임꼴) 스타일을 적용하여 실제 데이터와 명확히 구분한다.
- **구체적 가이드**: 단순 "내용 없음" 대신, 해당 필드에 입력할 내용을 구체적으로 안내하는 플레이스홀더(예: "고객 요청사항 기타 확인할 것")를 제공한다.
- **일관성**: 편집 모드(`textarea`, `input`)와 보기 모드 모두에서 동일한 플레이스홀더 문구를 유지하여 일관된 정보를 제공한다.
